...
# LINEAR REGRESSION ####
  ...
  registerdata <-   fread('SCAPIS/Gutsy/Phenotypes/Medication/Raw/SCAPIS-REGISTERSU-1715-LMED-20220511-T_R_LMED_27947_2021.txt', na.strings = c("NA",NA,""))
  ...
  # Atb after 
  registerdata_N0 <- registerdata[Subject %in% scapis$Subject[scapis$N_all==0], ]
  registerdata_N0 <- registerdata_N0[EDATUM>Visit1 & EDATUM<(Visit2+365.25),]
  atb_after_participants <- registerdata_N0[,.N, by = Subject][N==1, Subject]
...
linear.fun <- function(species, exposure, model = basic.model, df = class_atb_single){
  ...
  res_full$cohort <- "SCAPIS"
  
  res <- rbind(res,res_full)
  
  fwrite(res, file='results/scapis_species_singledose.tsv', sep = "\t")
  ...
  fwrite(res, file='results/scapis_alpha_singledose.tsv', sep = "\t")
