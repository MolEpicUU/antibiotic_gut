...
source('/Users/gabba126/Documents/PhD_projects/3.Antibiotic/revision_NatMed/revision__atb_scripts/order_species_by_taxa.R')
load("/Users/gabba126/Documents/PhD_projects/3.Antibiotic/revision_NatMed/revision__work/antibiotic_order_figures")
...
res <- rio::import('/Users/gabba126/Documents/PhD_projects/3.Antibiotic/revision_NatMed/revision__results/meta_species__sa_stratified_allantibiotics_agebelow55.tsv')
...
res <- res[which(res$model == "agebelow55"), ]
...
# users_ageabelow55 ####
users <- rio::import("/Users/gabba126/Documents/PhD_projects/3.Antibiotic/revision_NatMed/revision__work/count_antibiotics_nonzeros_byage_allcohorts.tsv")
users_ageabove55 <- users[, grep("abx|_ageabove55", colnames(users))]
users <- users[, grep("abx|_agebelow", colnames(users))]
...
users[users<=5] <- 0
users$users <- rowSums(users[, -1], na.rm = T)/4259
...
anno_ageabove55 <- rio::import('/Users/gabba126/Documents/PhD_projects/Microbiome/Taxonomy/Taxonomy_CHAMP.tsv')
res <- rio::import('/Users/gabba126/Documents/PhD_projects/3.Antibiotic/revision_NatMed/revision__results/meta_species__sa_stratified_allantibiotics_ageabove55.tsv')
res <- merge(res, anno_ageabove55[, c("MGS", "species")], by.x = "outcome", by.y = "MGS")
setnames(res, "q.value", "qvalue")

res <- res[which(res$model == "ageabove55"), ]
species <- unique(res$species)
res$beta[which(res$qvalue >= 0.05)] <- NA
...
# Users_ageabove55 ####
users <- users_ageabove55
users$abx <- gsub("Pen\\.", "Penicillin", users$abx)
users$abx <- gsub("Marcrolides", "Macrolides", users$abx)
users$abx <- gsub("Penicillin ES", "Penicillins ES", users$abx)

users[users<=5] <- 0
users$users <- rowSums(users[, -1], na.rm = T)/10715
users$abx <- gsub("0_1yr", "1yr", users$abx)
users$period <- stringr::str_extract(users$abx, "\\d.*yr")
users$abx <- trimws(gsub("\\d.*yr", "", users$abx))
...
ggsave('/Users/gabba126/Documents/PhD_projects/3.Antibiotic/revision_NatMed/revision__figures/fig_suppl_stratif_age.pdf', plot, width = 210, height = 148.5, unit = "mm")
