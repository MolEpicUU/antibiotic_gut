...
linear.fun <- function(species, exposure, model = basic.model){
  ...
  fwrite(res, file='results/scapis_species_linear_N1N2N3.tsv')
  ...
  fwrite(res, file='results/scapis_species_linear_class_N1N2N3.tsv')
  ...
  fwrite(res, file='results/scapis_species_linear_N1N2N3_Dis.tsv')
  ...
  res <- linear.fun(prevalent.species, 
                    exposure = atbclasses, 
                    model = full.model)
  res$model <-  "full.model"
  
  fwrite(res, file='results/scapis_species_linear_class_N1N2N3_Dis.tsv')
  
  
  message("End ful model")
